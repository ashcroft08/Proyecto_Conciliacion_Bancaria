...
export const RegisterJefeContador = () => {
  const { users, getUserJefeContable, getUser, updateUser, deleteUser } =
    useUser();
  const { user } = useAuth();
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      if (!hasFetchedUsers.current) {
        ...
        await getUserJefeContable();
        hasFetchedUsers.current = true; // Marca que ya se han obtenido los usuarios
      }
    };
    fetchUsers();
  }, [getUserJefeContable]); // Solo se ejecuta una vez al montar el componente
  ...
  const onSubmit = async (values) => {
    ...
    if (success) {
      CustomToast("¡Jefe contable registrado exitosamente!", "success");
      setVisible(false); // Cierra el modal
      await getUserJefeContable(); // Actualiza la lista de usuarios después de registrar uno nuevo
  ...
  const onSubmitEdit = async (values) => {
    try {
      ...
      if (success) {
        CustomToast("¡Jefe contable actualizado exitosamente!", "success");
        setEditVisible(false);
        await getUserJefeContable(); // Actualizar lista
      }
    } catch (error) {
      CustomToast(
  ...
  const handleConfirmDelete = async () => {
    ...
    if (success) {
      ...
      CustomToast("¡Jefe contable eliminado exitosamente!", "success");
  ...
  return (
    <>
      ...
      <CCard>
        <CCardHeader>
          <div className="d-flex justify-content-end mt-1">
            <CInputGroup>
              ...
              {users.length > 0 ? ( // Renderizar el popover solo si el botón está deshabilitado
                <CPopover
                  content="Jefe contable agregado, botón desactivado"
                  placement="right"
                  trigger={["hover", "focus"]}
                >
                  <span className="d-inline-block" tabIndex={0}>
                    <CButton
                      ...
                      className="fw-bold"
                      disabled={users.length > 0} // Deshabilitar el botón si hay registros
                    >
                      Agregar un jefe contable
                    </CButton>
                  </span>
                </CPopover>
              ) : (
        ...
        {/* Modal para crear nuevo jefe contable */}
        <CModal
          backdrop="static"
          alignment="center"
          visible={visible}
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <CModalHeader>
              <CModalTitle id="VerticallyCenteredExample" className="fw-bold">
                Ingresar los datos del jefe contable
              </CModalTitle>
            </CModalHeader>
            <CModalBody>
          ...
          <form onSubmit={handleSubmitEdit(onSubmitEdit)}>
            <CModalHeader>
              <CModalTitle className="fw-bold">
                Editar los datos del jefe contable
              </CModalTitle>
            </CModalHeader>
            <CModalBody>
              <div className="mt-2">
        ...
        >
          <CModalHeader>
            <CModalTitle id="DeleteUser Modal" className="fw-bold">
              Eliminar al jefe contable
            </CModalTitle>
          </CModalHeader>
          <CModalBody>
            <p>
              ¿Estás seguro de que deseas eliminar los datos de {currentUser?.nombres}?
            </p>
          </CModalBody>
          <CModalFooter>
